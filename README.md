...
All objectives load the exact same saved initial weights, including the initialized head. No head-only warmup is currently used. Full text parameters plus the head are optimized with AdamW and BF16 autocast; FP32 parameter/optimizer storage avoids low-precision Adam updates. Training uses gradient checkpointing, `use_cache=False`, clipping at 1.0, linear warmup/decay, separate backbone/head LRs of 2e-5/2e-4, weight decay .01, seed 17, effective batch 16, microbatch 4 (increased from the initial 2 after a Slurm throughput check), and max input length 512. Overlong inputs raise instead of silently truncating the candidate. A single logical batch is retried with a smaller microbatch on activation OOM. No LoRA, quantization, offload, or ZeRO is used.
