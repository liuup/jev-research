...
import torch
import yaml
from jev2048.dataset import read_rows,collate
from jev2048.model import JevModel,load_tokenizer
from jev2048.objectives import loss
from jev2048.utils import seed_all,save_json

if __name__ == "__main__":
    assert os.environ.get("SLURM_JOB_ID"),"GPU work must run under Slurm"
    torch.set_num_threads(4)
    config=yaml.safe_load(open("configs/model.yaml")); print(config,flush=True)
    seed_all(config["seed"]); torch.cuda.reset_peak_memory_stats()
    ...
    report=dict(success=True,peak_gpu_gib=torch.cuda.max_memory_allocated()/2**30,
        text_parameters=sum(p.numel() for p in model.backbone.parameters()),
        head_parameters=sum(p.numel() for p in model.head.parameters()),vision_parameters=0,
        vision_excluded=True,lm_head_excluded=True,probabilities=lp.exp().detach().cpu().tolist())
    save_json("results/gpu_smoke.json",report); print(json.dumps(report),flush=True)
