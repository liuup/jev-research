...
class JevModel(nn.Module):
    ...
    def load_base(cls, config):
            ...
            with safe_open(root / filename, framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key.startswith(prefix):
                        state[key[len(prefix) :]] = f.get_tensor(key).to(torch.bfloat16)
        ...
        return cls(
            backbone,
            DecisionHead(
                text_config.hidden_size, config["head_width"], config["attention_heads"]
            ),
        )
    ...
    def forward(self, batch):
        ...
        positions = torch.arange(hidden.shape[1], device=device).expand(
            hidden.shape[:2]
        )
        last = (
            positions.masked_fill(~tokens["attention_mask"].bool(), -1).max(-1).values
        )
        pooled = hidden[torch.arange(hidden.shape[0], device=device), last].float()
        offsets = batch["offsets"]
        mask = batch["mask"].to(device)
        groups = torch.nn.utils.rnn.pad_sequence(
            [pooled[a:b] for a, b in zip(offsets[:-1], offsets[1:])], batch_first=True
        )
