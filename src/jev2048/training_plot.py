"""Plot metrics recorded in a training run's JSONL log."""
...
def plot_metrics(records, output, smooth, title, phase_boundaries=()):
    ...
    if "predicted_success_probability" in records[0]["train"]:
        definitions = definitions[:2] + (
            ("predicted_success_probability", "Mean P(reach 2048)"),
        )
...
def plot_training_run(run, output=None, smooth=25, title=None):
    if smooth < 1:
    ...
    if continuation_from:
        ...
        records = list(
            {row["step"]: row for row in parent_records + records}.values()
        )
        records.sort(key=lambda row: row["step"])
    objective = config["objective"].replace("_", "-")
    mode = config.get("mode", "training").title()
    resolved_title = title or f"{mode} {objective} probability metrics"
    plot_metrics(records, output, smooth, resolved_title, phase_boundaries)
    return dict(output=str(output), steps=len(records), smooth=smooth)
