import pytest
from jev2048.env import Game, merge, moved, ACTIONS
from jev2048.heuristic import HeuristicPolicy

@pytest.mark.parametrize("row,out,score", [
    ([2,2,2,2],[4,4,0,0],8), ([4,4,8,8],[8,16,0,0],24),
    ([2,2,4,0],[4,4,0,0],4), ([4,0,4,4],[8,4,0,0],8),
    ([0,0,0,0],[0,0,0,0],0), ([2,4,8,16],[2,4,8,16],0),
    ([2,2,4,4],[4,8,0,0],12), ([2,0,0,2],[4,0,0,0],4)])
...
def test_directions(action):
    ...
    if action == "LEFT": assert b[0] == (4,0,0,0)
    if action == "RIGHT": assert b[0] == (0,0,0,4)
    if action == "UP": assert b[0] == (4,4,0,0) and b[3] == (0,0,0,0)
    if action == "DOWN": assert b[3] == (4,4,0,0) and b[0] == (0,0,0,0)
...
def test_seed_clone_invalid_terminal():
    ...
    for _ in range(30):
        ...
        c = a.clone(); action = p.choose(a)
        a.step(action); b.step(action); c.step(action)
    ...
    assert full.state() == before.state() and full.rng.getstate() == before.rng.getstate()
